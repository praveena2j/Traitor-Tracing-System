...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Considering a test image as input for recognition
% imtest=input('Enter the test image number ---> ');
% teststr=int2str(imtest);
% pathoftestim=strcat(teststr,'.jpg');
% testimage=strcat('C:\Users\JESUS\Desktop\TestDatabase\',pathoftestim);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Creating 1d array for the 2d input image
test=chtimg;
% test=rgb2gray(imread(testimage));
% test=imresize(test,[b(1) b(2)]);
for i=1:b(1)*b(2)
...
stem(euc);
 ...
 % hist(euc,3);
% title('Test Image');
% % if mindis<5.2605e+007
% %%Displaying the test and recognised images
% recim=strcat(dbpath,'\',int2str(arg_min),'.jpg');
% figure;
% imshow(recim);
% title('Recognized Image');
% else disp('image is not in the data base');
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
